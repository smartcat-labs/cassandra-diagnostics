...
public class ConnectorProxyImpl implements ConnectorProxy {
    ...
    private static ActorRef connector;

    @Override
    public ActorRef init(Instrumentation inst, ActorSystem system, Configuration configuration) {
        this.configuration = configuration;

        try {
            this.connector = system
                    .actorOf(ActorFactory.connectorProps(this.configuration.connector.connector, this.configuration));
        } catch (Exception e) {
            logger.error("Failed to create connector actor.", e);
            throw new RuntimeException("Failed to create connector actor.");
        }

        queryProcessorWrapper = new QueryProcessorWrapper(this.connector, configuration.connector);
     ...
     */
    private void setQueryProcessorIntercepter(Instrumentation inst) {
        ...
        new AgentBuilder.Default().with(byteBuddy).with(RedefinitionStrategy.RETRANSFORMATION)
                ...
                .transform(new Transformer() {
                    ...
                    public Builder<?> transform(Builder<?> builder, TypeDescription typeDescription,
                        ...
                        return builder.visit(Advice.to(ProcessPreparedAdvice.class).on(named("processPrepared")
            ...
            public Builder<?> transform(Builder<?> builder, TypeDescription typeDescription, ClassLoader classLoader) {
                return builder.visit(Advice.to(ProcessAdvice.class).on(named("process")
                        .and(takesArguments(queryStringDescription(), queryStateDescription(),
                                queryOptionsDescription()))
                        .and(returns(named("org.apache.cassandra.transport.messages.ResultMessage")))));
            }
        }).transform(new Transformer() {
            @Override
            public Builder<?> transform(Builder<?> builder, TypeDescription typeDescription, ClassLoader classLoader) {
                return builder.visit(Advice.to(StorePreparedAdvice.class).on(named("storePreparedStatement")));
            }
        }).installOn(inst);
    }
     ...
     * @param inst instrumentation handle
     */
    private void setCassandraDaemonIntercepter(Instrumentation inst) {
    ...
    public static class ProcessPreparedAdvice {
        ...
        public static void exit(
                ...
                @Advice.FieldValue(value = "preparedStatements")
                        ConcurrentLinkedHashMap<MD5Digest, ParsedStatement.Prepared> preparedStatements) {
            ConnectorProxyImpl.queryProcessorWrapper
    ...
    public static class ProcessAdvice {
        ...
        public static void exit(
                ...
                @Advice.Return
                        ResultMessage result) {
            ConnectorProxyImpl.queryProcessorWrapper.process(queryString, queryState, options, startTime, result);
        }
    }
    ...
    private static TypeDescription queryStringDescription() {
        return new TypeDescription.Latent("java.lang.String", Modifier.INTERFACE, null, null);
    }
    ...
    public static class CassandraDaemonAdvice {
        ...
        @Advice.OnMethodExit
        public static void exit() {
            ConnectorProxyImpl.cassandraSetupComplete();
        }
    }
}
